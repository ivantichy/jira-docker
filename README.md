...
`docker run -d --name jira -p 8080:8080  llowell/jira-yakkety:7.3.0`
...
To stop it run `docker stop jira`. To run it again use `docker start jira`. To clean all your settings (to uninstall JIRA) run `docker rm jira`. Then to run a new container use "run" command above e.g. `docker run -d --name jira -p 8080:8080  llowell/jira-yakkety:7.3.0`.
...
When you run container using command like mentioned above (Scenario A) `docker run -d --name jira -p 8080:8080  llowell/jira-yakkety:7.3.0` your database data, JIRA home directory containing attachments, backups etc and JIRA application directory are stored using volumes on host machine (not inside the container). You can find information about physical location using `docker inspect jira`. To find volumes location look for "mount" section in the printed output.

## To use your own path for app data

I personally start the container using this command: `docker run --cidfile ${PROJECT_DIR}/jiracid --rm -p 8080:8080 -v ${DATA_DIR}/postgres:/var/lib/postgresql/9.5/main -v ${DATA_DIR}/jira-app:/var/atlassian/jira-app -v ${DATA_DIR}/jira-home:/var/atlassian/jira-home llowell/jira-yakkety:7.3.0 "$@" &`. This causes (`-v` paramater) that Docker daemon uses paths I selected (`${DATA_DIR}/`). I usually backup these folders and I use them to migrate JIRA from one location to another. These folders survive container deletion which is important.

## How to set it up

1. On the Docker host machine create these folders:
 * `mkdir -p ${DATA_DIR}/postgres`
 * `mkdir -p ${DATA_DIR}/jira-app`
 * `mkdir -p ${DATA_DIR}/jira-home`

2. Download start script executing: `wget https://raw.githubusercontent.com/llowell/jira-yakkety-docker/7.3.0/runjira.sh -O ${PROJECT_DIR}/runjira.sh && chmod +x ${PROJECT_DIR}/runjira.sh`

3. Install JIRA (will not start JIRA)  executing `${PROJECT_DIR}/runjira.sh install`. Installation is running in background. Please check state using `docker ps` to see when it is finished (container exits). It takes something like a minute or so.
...
5. Run JIRA using this command: `${PROJECT_DIR}/runjira.sh`. Container will set owner on folders from step 1 (postgres 1100:1100, jira-home and jira-app 1200:1200) so count with that. This is needed because JIRA and database is not running as root. You can stop container anytime using `${PROJECT_DIR}/stopjira.sh` command. This will gracefully stop JIRA and PostgreSQL service inside the container, container will stop, exit and delete itself (not data) after that.
...
Anytime you can run `${PROJECT_DIR}/runjira.sh purge`. This will permanently erase all your data. You can use it to start from scratch (clean JIRA installation and clean database).
...
2. Stop old JIRA instance. Back up your JIRA home and JIRA application directory. Copy JIRA home directory to ${DATA_DIR}/jira-home on your docker host machine.
